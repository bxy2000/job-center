		...
		registryOrRemoveThreadPool = new ThreadPoolExecutor(
				...
				new ThreadFactory() {
					...
					public Thread newThread(Runnable r) {
						return new Thread(r, "job-center, admin JobRegistryHelper-registryOrRemoveThreadPool-" + r.hashCode());
					}
				},
				new RejectedExecutionHandler() {
					...
					public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
						r.run();
						logger.warn(">>>>>>>>>>> job-center, registry or remove too fast, match threadpool rejected handler(run now).");
					}
				});
			...
			public void run() {
				while (!toStop) {
					try {
						...
						if (!toStop) {
							logger.error(">>>>>>>>>>> job-center, job registry monitor thread error:{}", e);
						}
					}
					try {
				...
				logger.info(">>>>>>>>>>> job-center, job registry monitor thread stop");
			}
		});
		registryMonitorThread.setDaemon(true);
		registryMonitorThread.setName("job-center, admin JobRegistryMonitorHelper-registryMonitorThread");
